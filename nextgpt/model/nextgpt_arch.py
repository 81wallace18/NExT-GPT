...
DEFAULT_DECODER_KINDS = ("image", "video", "audio")
...
class NextGPTMetaModel:
    def __init__(self, config):
        ...
        self.decoder_lazy_load = getattr(config, "decoder_lazy_load", False)
        self.decoder_device = getattr(config, "decoder_device", None)
        self.decoder_dtype = getattr(config, "decoder_dtype", None)
        self.decoder_offload = getattr(config, "decoder_offload", None)
        self.decoder_attention_slicing = getattr(
            config, "decoder_attention_slicing", None
        )
        self.decoder_xformers = getattr(config, "decoder_xformers", None)

        if hasattr(config, "image_decoder"):
            if self.decoder_lazy_load:
                self.image_decoder = None
            else:
                self.image_decoder = builder_decoder(config, "image")
            mm_output_img_projector_type = getattr(
                config, "mm_output_img_projector_type", "transformer"
            )
            mm_output_img_features = getattr(
                config, "mm_output_img_features", 1024
            )  # 768- v1.5, 1024- v2.0
            mm_output_img_num_query_token = getattr(
                config, "mm_output_img_num_query_token", 77
            )
            self.mm_output_img_projector = build_output_projector(
                config,
                mm_output_img_projector_type,
                mm_output_img_features,
                mm_output_img_num_query_token,
            )
        ...
        if hasattr(config, "video_decoder"):
            ...
            mm_output_vid_projector_type = getattr(
                config, "mm_output_vid_projector_type", "transformer"
            )
            mm_output_vid_features = getattr(config, "mm_output_vid_features", 1024)
            mm_output_vid_num_query_token = getattr(
                config, "mm_output_vid_num_query_token", 77
            )
            self.mm_output_vid_projector = build_output_projector(
                config,
                mm_output_vid_projector_type,
                mm_output_vid_features,
                mm_output_vid_num_query_token,
            )
        ...
        if hasattr(config, "audio_decoder"):
            ...
            mm_output_aud_projector_type = getattr(
                config, "mm_output_aud_projector_type", "transformer"
            )
            mm_output_aud_features = getattr(config, "mm_output_aud_features", 512)
            mm_output_aud_num_query_token = getattr(
                config, "mm_output_aud_num_query_token", 1
            )
            self.mm_output_aud_projector = build_output_projector(
                config,
                mm_output_aud_projector_type,
                mm_output_aud_features,
                mm_output_aud_num_query_token,
            )
    ...
    def get_image_decoder(self):
        ...
        if image_decoder is None and self.decoder_lazy_load:
            self._apply_decoder_config(self.config)
            self.image_decoder = builder_decoder(self.config, "image")
            image_decoder = self.image_decoder
        return image_decoder

    def _apply_decoder_config(self, config):
        if self.decoder_device is not None:
            config.decoder_device = self.decoder_device
        if self.decoder_dtype is not None:
            config.decoder_dtype = self.decoder_dtype
        if self.decoder_offload is not None:
            config.decoder_offload = self.decoder_offload
        if self.decoder_attention_slicing is not None:
            config.decoder_attention_slicing = self.decoder_attention_slicing
        if self.decoder_xformers is not None:
            config.decoder_xformers = self.decoder_xformers
    ...
    def get_video_decoder(self):
        ...
        if video_decoder is None and self.decoder_lazy_load:
            self._apply_decoder_config(self.config)
            self.video_decoder = builder_decoder(self.config, "video")
    ...
    def get_audio_decoder(self):
        ...
        if audio_decoder is None and self.decoder_lazy_load:
            self._apply_decoder_config(self.config)
            self.audio_decoder = builder_decoder(self.config, "audio")
    ...
    def initialize_input_multimodal_modules(self, model_args, fsdp=None):
        multimodal_tower = getattr(
            model_args,
            "multimodal_input_tower",
            getattr(model_args, "multimodal_tower", None),
        )
        pretrain_mm_input_adapter = getattr(
            model_args, "pretrain_mm_input_adapter", None
        )
        ...
        self.config.mm_input_projector_type = getattr(
            model_args, "mm_input_projector_type", "linear"
        )
        ...
        if pretrain_mm_input_adapter is not None:
            print(
                "Loading pretrain_mm_input_adapter from : ", pretrain_mm_input_adapter
            )
            mm_projector_weights = torch.load(
                pretrain_mm_input_adapter, map_location="cpu"
            )
            ...
            self.mm_input_projector.load_state_dict(
                state_dict=get_w(mm_projector_weights, "mm_input_projector")
            )
    ...
    def initialize_output_multimodal_modules(self, model_args):
        ...
        self.config.mm_output_img_projector_type = getattr(
            model_args, "mm_output_img_projector_type", "transformer"
        )
        self.config.mm_output_vid_projector_type = getattr(
            model_args, "mm_output_vid_projector_type", "transformer"
        )
        self.config.mm_output_aud_projector_type = getattr(
            model_args, "mm_output_aud_projector_type", "transformer"
        )

        self.config.mm_output_img_features = getattr(
            model_args, "mm_output_img_features", 1024
        )
        self.config.mm_output_img_num_query_token = getattr(
            model_args, "mm_output_img_num_query_token", 77
        )

        self.config.mm_output_vid_features = getattr(
            model_args, "mm_output_vid_features", 1024
        )
        self.config.mm_output_vid_num_query_token = getattr(
            model_args, "mm_output_vid_num_query_token", 77
        )

        self.config.mm_output_aud_features = getattr(
            model_args, "mm_output_aud_features", 512
        )
        self.config.mm_output_aud_num_query_token = getattr(
            model_args, "mm_output_aud_num_query_token", 1
        )

        pretrain_mm_output_img_adapter = getattr(
            model_args, "pretrain_mm_output_img_adapter", None
        )
        pretrain_mm_output_vid_adapter = getattr(
            model_args, "pretrain_mm_output_vid_adapter", None
        )
        pretrain_mm_output_aud_adapter = getattr(
            model_args, "pretrain_mm_output_aud_adapter", None
        )
        ...
        if getattr(self, "mm_output_img_projector", None) is None:
            ...
            self.mm_output_img_projector = build_output_projector(
                self.config,
                self.config.mm_output_img_projector_type,
                self.config.mm_output_img_features,
                self.config.mm_output_img_num_query_token,
            )
        else:
        ...
        if pretrain_mm_output_img_adapter is not None:
            print(
                "Loading pretrain_mm_output_img_adapter from : ",
                pretrain_mm_output_img_adapter,
            )
            mm_output_img_projector_weights = torch.load(
                pretrain_mm_output_img_adapter, map_location="cpu"
            )
            ...
            self.mm_output_img_projector.load_state_dict(
                state_dict=get_w(
                    mm_output_img_projector_weights, "mm_output_img_projector"
                )
            )
        ...
        if getattr(self, "mm_output_vid_projector", None) is None:
            ...
            self.mm_output_vid_projector = build_output_projector(
                self.config,
                self.config.mm_output_vid_projector_type,
                self.config.mm_output_vid_features,
                self.config.mm_output_vid_num_query_token,
            )
        else:
        ...
        if pretrain_mm_output_vid_adapter is not None:
            print(
                "Loading pretrain_mm_output_vid_adapter from : ",
                pretrain_mm_output_vid_adapter,
            )
            mm_output_vid_projector_weights = torch.load(
                pretrain_mm_output_vid_adapter, map_location="cpu"
            )
            ...
            self.mm_output_vid_projector.load_state_dict(
                state_dict=get_w(
                    mm_output_vid_projector_weights, "mm_output_vid_projector"
                )
            )
        ...
        if getattr(self, "mm_output_aud_projector", None) is None:
            ...
            self.mm_output_aud_projector = build_output_projector(
                self.config,
                self.config.mm_output_aud_projector_type,
                self.config.mm_output_aud_features,
                self.config.mm_output_aud_num_query_token,
            )
        else:
        ...
        if pretrain_mm_output_aud_adapter is not None:
            print(
                "Loading pretrain_mm_output_aud_adapter from : ",
                pretrain_mm_output_aud_adapter,
            )
            mm_output_aud_projector_weights = torch.load(
                pretrain_mm_output_aud_adapter, map_location="cpu"
            )
            ...
            self.mm_output_aud_projector.load_state_dict(
                state_dict=get_w(
                    mm_output_aud_projector_weights, "mm_output_aud_projector"
                )
            )
...
class NextGPTMetaForCausalLM(ABC):
    ...
    def encode_images(self, images):
        image_features = self.get_model().get_multimodal_tower()(
    ...
    def encode_videos(self, videos):
        video_features = self.get_model().get_multimodal_tower()(
    ...
    def encode_audios(self, audios):
        audio_features = self.get_model().get_multimodal_tower()(
            audios, modality="audio"
        )
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if (
            multimodal_tower is None
            or (images is None and videos is None and audios is None)
        ...
        if images is not None:
            if type(images) is list or images.ndim == 5:
                if type(images) is list:
                    images = [
                        (x.unsqueeze(dim=0) if x.ndim == 3 else x) for x in images
                    ]
            ...
            image_features = torch.zeros(
                (input_ids.shape[0], 4096),
                dtype=multimodal_tower.dtype,
        ...
        if videos is not None:
            if type(videos) is list or videos.ndim == 7:
                if type(videos) is list:
                    videos = [
                        (x.unsqueeze(dim=0) if x.ndim == 5 else x) for x in videos
                    ]
        ...
        if audios is not None:
            if type(audios) is list or audios.ndim == 6:
                if type(audios) is list:
                    audios = [
                        (x.unsqueeze(dim=0) if x.ndim == 4 else x) for x in audios
                    ]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            video_token_indices = torch.where(cur_input_ids == VIDEO_TOKEN_INDEX)[
                0
            ].tolist()
            audio_token_indices = torch.where(cur_input_ids == AUDIO_TOKEN_INDEX)[
                0
            ].tolist()
            _specfical_token_indices = (
                image_token_indices + video_token_indices + audio_token_indices
            )
            _specfical_token_indices.sort()
            _specfical_token_indices = (
                [-1] + _specfical_token_indices + [cur_input_ids.shape[0]]
            )
            ...
            for i in range(len(_specfical_token_indices) - 1):
                cur_input_ids_noim.append(
                    cur_input_ids[
                        _specfical_token_indices[i] + 1 : _specfical_token_indices[
                            i + 1
                        ]
                    ]
                )
            ...
            cur_input_embeds = self.get_model().embed_tokens(
                torch.concat(cur_input_ids_noim)
            )
            cur_input_embeds_no_im = torch.split(cur_input_embeds, split_sizes, dim=0)
            cur_new_input_embeds = []
            cur_new_labels = []

            for i, _token_indices in zip(
                range(num_images + num_videos + num_audios + 1),
                _specfical_token_indices[1:],
            ):
                ...
                if _token_indices in image_token_indices:
                    ...
                    cur_mm_features = (
                        cur_mm_features.unsqueeze(0)
                        if cur_mm_features.ndim == 1
                        else cur_mm_features
                    )
                    cur_image_idx += 1
    ...
    def initialize_vision_tokenizer(self, model_args, tokenizer):
        ...
        # add modality tokens
        signal_token_list = []
        signal_token_list.extend(
            [f"<image_{i:02d}>" for i in range(model_args.n_img_tokens)]
        )
        signal_token_list.extend(
            [f"<video_{i:02d}>" for i in range(model_args.n_vid_tokens)]
        )
        signal_token_list.extend(
            [f"<audio_{i:02d}>" for i in range(model_args.n_aud_tokens)]
        )
        ...
        if num_new_tokens > 0:
            ...
            print(
                ...
                self.get_input_embeddings().weight.requires_grad,
            )
        ...
        if (
            model_args.tune_mm_input_adapter
            or model_args.tune_mm_output_img_adapter
            or model_args.tune_mm_output_vid_adapter
            or model_args.tune_mm_output_aud_adapter
        ):
        ...
        if (
            getattr(model_args, "pretrain_mm_output_img_adapter", None) is not None
            or getattr(model_args, "pretrain_mm_output_vid_adapter", None) is not None
            or getattr(model_args, "pretrain_mm_output_aud_adapter", None) is not None
        ):
            output_adapter = None
            if getattr(model_args, "pretrain_mm_output_img_adapter", None) is not None:
                ...
                getattr(model_args, "pretrain_mm_output_vid_adapter", None) is not None
            ):
                output_adapter = model_args.pretrain_mm_output_vid_adapter
            elif (
                getattr(model_args, "pretrain_mm_output_aud_adapter", None) is not None
            ):
        ...
        if model_args.mm_use_img_start_end:
            ...
            if model_args.pretrain_mm_input_adapter:
                mm_projector_weights = torch.load(
                    path=model_args.pretrain_mm_input_adapter
                )
                embed_tokens_weight = mm_projector_weights["model.embed_tokens.weight"]
                assert num_new_tokens == 2
                if input_embeddings.shape == embed_tokens_weight.shape:
