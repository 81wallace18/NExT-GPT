...
try:
    ...
    if _functional is not None:
        ...
        shim.__dict__.update(_functional.__dict__)
...
class ImageProcessor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.image_mean = (
            (0.48145466, 0.4578275, 0.40821073) if image_mean is None else image_mean
        )
        self.image_std = (
            (0.26862954, 0.26130258, 0.27577711) if image_std is None else image_std
        )
...
class AudioProcessor(nn.Module):
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        target_length = (
            target_length if target_length is not None else self.target_length
        )
        sample_rate = sample_rate if sample_rate is not None else self.sample_rate
        clip_duration = (
            clip_duration if clip_duration is not None else self.clip_duration
        )
        clips_per_video = (
            clips_per_video if clips_per_video is not None else self.clips_per_video
        )
...
class VideoProcessor(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        # )
        self.video_mean = (
            (0.48145466, 0.4578275, 0.40821073) if video_mean is None else video_mean
        )
        self.video_std = (
            (0.26862954, 0.26130258, 0.27577711) if video_std is None else video_std
        )
