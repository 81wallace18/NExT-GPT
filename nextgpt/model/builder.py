...
def load_pretrained_model(
    ...
    decoder_lazy_load=False,
    decoder_device=None,
    decoder_dtype=None,
    decoder_offload=None,
    decoder_attention_slicing=None,
    decoder_xformers=None,
    **kwargs,
):
    ...
    if "nextgpt" in model_name.lower():
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            model = NextGPTLlamaForCausalLM.from_pretrained(
                model_base, config=cfg_pretrained
            ).to(device="cuda", dtype=torch.float16)
            print("kwargs: ", kwargs)

            print(
                "mm_input_projector device", model.get_model().mm_input_projector.device
            )
            print(
                "mm_input_projector dtype", model.get_model().mm_input_projector.dtype
            )
            print(
                "mm_output_img_projector device",
                model.get_model().mm_output_img_projector.device,
            )
            print(
                "mm_output_img_projector dtype",
                model.get_model().mm_output_img_projector.dtype,
            )
    ...
    if "nextgpt" in model_name.lower():
        if decoder_lazy_load:
            model.config.decoder_lazy_load = True
        if decoder_device is not None:
            model.config.decoder_device = decoder_device
        if decoder_dtype is not None:
            model.config.decoder_dtype = decoder_dtype
        if decoder_offload is not None:
            model.config.decoder_offload = decoder_offload
        if decoder_attention_slicing is not None:
            model.config.decoder_attention_slicing = decoder_attention_slicing
        if decoder_xformers is not None:
            model.config.decoder_xformers = decoder_xformers
        ...
        mm_output_img_projector = model.get_output_image_projector().to(
            device=model.device
        )
        mm_output_vid_projector = model.get_output_video_projector().to(
            device=model.device
        )
        mm_output_aud_projector = model.get_output_audio_projector().to(
            device=model.device
        )
        image_decoder = model.get_image_decoder()
        if image_decoder is not None:
            image_decoder = image_decoder.to(device=model.device)
        video_decoder = model.get_video_decoder()
        if video_decoder is not None:
            video_decoder = video_decoder.to(device=model.device)
        audio_decoder = model.get_audio_decoder()
        if audio_decoder is not None:
            audio_decoder = audio_decoder.to(device=model.device)
